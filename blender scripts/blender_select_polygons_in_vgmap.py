# Small add-on to select polygons included in vgmap file. Run inside Blender.
# GitHub eArmada8/gust_stuff

bl_info = {
    "name": "Select polygons using VGMap",
    "description": "Small tool to select polygons (or vertices) that belong to weight groups detailed in a VGMap.  Only the joint names are used, not the index numbers.",
...
class VertexMatch(Operator, ImportHelper):
    ...
    face_mode: bpy.props.BoolProperty(name = "Select intact polygons",\
        description = 'Default behavior is to select polygons, uncheck to select individual vertices instead', default = True)
    ...
    def execute(self, context):
        ...
        if ob.type == 'MESH':
            ...
            bpy.ops.mesh.select_mode(type={True:'FACE', False:'VERT'}[self.face_mode])
            if self.unselect_if_not_in_vgmap == True:
                bpy.ops.mesh.select_all(action='DESELECT')
            bpy.ops.object.mode_set(mode='OBJECT')
            if self.face_mode == True:
                for i in range(len(mesh.data.polygons)):
                    if self.reverse_select == False and self.select_only_if_all_present == True:
                        if any([all(x in vgmapbones[j] for x in [vg[x.group] for y in mesh.data.polygons[i].vertices\
                                for x in mesh.data.vertices[y].groups]) for j in range(len(vgmapbones))]):
                            mesh.data.polygons[i].select = True
                    elif self.reverse_select == False and self.select_only_if_all_present == False:
                        if any([any(x in vgmapbones[j] for x in [vg[x.group] for y in mesh.data.polygons[i].vertices\
                                for x in mesh.data.vertices[y].groups]) for j in range(len(vgmapbones))]):
                            mesh.data.polygons[i].select = True
                    elif self.reverse_select == True and self.select_only_if_all_present == True:
                        if not any([all(x in vgmapbones[j] for x in [vg[x.group] for y in mesh.data.polygons[i].vertices\
                                for x in mesh.data.vertices[y].groups]) for j in range(len(vgmapbones))]):
                            mesh.data.polygons[i].select = True
                    elif self.reverse_select == True and self.select_only_if_all_present == False:
                        if not any([any(x in vgmapbones[j] for x in [vg[x.group] for y in mesh.data.polygons[i].vertices\
                                for x in mesh.data.vertices[y].groups]) for j in range(len(vgmapbones))]):
                            mesh.data.polygons[i].select = True
            else:
...
class VertexMatchMenu(bpy.types.Operator):
    ...
    bl_label = "Select polygons using VGMap"
    bl_description = "Select polygons (or vertices) that belong to weight groups detailed in a VGMap.  Only the joint names are used, not the index numbers"
    bl_options = {'REGISTER'}
